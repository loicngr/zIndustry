...
export class Map {
  ...
  constructor(mapConfig: TMapConfig, tileConfig: TTileConfig) {
    ...
    this.tileConfig.tilesWithCollide = this.tileConfig.tiles
      .filter((i) => {
        return i.properties.find(
          (ii) => ii.name === "collide" && ii.value === true
        );
      })
      .map((i) => i.id + 1);
  }
  ...
  public setTile(
    ...
    this.mapConfig.layers[index].data[height * this.mapConfig.width + width] =
      tile;
  }
  ...
  public getTile(index: number, width: number, height: number): number {
    return this.mapConfig.layers[index].data[
      height * this.mapConfig.width + width
    ];
  }
}
